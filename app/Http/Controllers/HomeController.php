...
class HomeController extends Controller {
  ...
  public function passwordReset () {
    ...
    if ($validator->fails()) {
      ...
      if (!$user) {
        ...
        $body =
          str_replace('{resetLink}', Setting::where('setting_key', 'portal_address')->first()->setting_value . '/reset/' . $user->id . '/' . $code, $body);
        Mail::raw($body, function ($message) use ($user) {
          ...
          $message->setSubject(Setting::where('setting_key', 'password_reset_subject')->first()->setting_value);
        });
        Flash::success(trans('login.reset_sent'));
        return redirect()->back()->withSuccess(trans('login.reset_sent'));
      }
  ...
  public function completeReset (Request $request, $id, $code) {
    ...
    if ($validator->fails()) {
      ...
      return redirect()->back()->withSuccess(trans('login.reset_success'));
  ...
  public function processClientLogin (Request $request) {
    if (Borrower::where('username', $request->username)->where('password', md5($request->password))->count() == 1) {
      $borrower =
        Borrower::where('username', $request->username)->where('password', md5($request->password))->first();
