  ...
  public function store (Request $request) {
    ...
    if (!empty($request->file('files'))) {
      ...
      foreach ($request->file('files') as $key) {
        $file = ['files' => $key];
        $rules = ['files' => 'required|mimes:jpeg,jpg,bmp,png,pdf,docx,xlsx'];
        ...
        if ($validator->fails()) {
          ...
          $files[$count] = $fname;
          $key->move(public_path() . '/uploads', $fname);
        }
        $count++;
      }
    }
    ...
    if ($loan->repayment_cycle == 'daily') {
      $repayment_cycle = 'day';
      $loan->maturity_date =
        date_format(date_add(date_create($request->first_payment_date), date_interval_create_from_date_string($period . ' days')), 'Y-m-d');
    }
    if ($loan->repayment_cycle == 'weekly') {
      $repayment_cycle = 'week';
      $loan->maturity_date =
        date_format(date_add(date_create($request->first_payment_date), date_interval_create_from_date_string($period . ' weeks')), 'Y-m-d');
    }
    if ($loan->repayment_cycle == 'monthly') {
      $repayment_cycle = 'month';
      $loan->maturity_date =
        date_format(date_add(date_create($request->first_payment_date), date_interval_create_from_date_string($period . ' months')), 'Y-m-d');
    }
    ...
    if ($loan->repayment_cycle == 'yearly') {
      ...
      $loan->maturity_date =
        date_format(date_add(date_create($request->first_payment_date), date_interval_create_from_date_string($period . ' years')), 'Y-m-d');
    }
    $loan->save();
